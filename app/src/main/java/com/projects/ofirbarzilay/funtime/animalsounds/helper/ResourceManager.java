    ...
    private ResourceManager(Activity activity){
        ...
        try {
            ...
            for (int i = 0; i < animalNamesArray.length(); i++) {
                ...
                identifier = mActivity.getResources().getIdentifier(name, "drawable", "com.projects.ofirbarzilay.funtime");
                if (identifier > 0) {
                    animal.setPictureID(identifier);
                }

                identifier = mActivity.getResources().getIdentifier(name, "raw", "com.projects.ofirbarzilay.funtime");
                if (identifier > 0) {
                    animal.setSoundID(identifier);
                }

                identifier = mActivity.getResources().getIdentifier("name_" + name, "raw", "com.projects.ofirbarzilay.funtime");
                if (identifier > 0) {
                    animal.setNameSoundID(identifier);
                }

                identifier = mActivity.getResources().getIdentifier(name, "string", "com.projects.ofirbarzilay.funtime");
                if (identifier > 0) {
